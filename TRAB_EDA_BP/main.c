...
void gerar_arquivo_de_dados(const char* nome_arquivo) {
    ...
    if (!file) {
        perror("Nao foi possivel criar o arquivo de dados de origem");
        return;
    }
    ...
    fclose(file);
    printf("Arquivo de dados '%s' gerado com 10.000 registros!\n", nome_arquivo);
}
...
int main(void) {
    srand(time(NULL));
    printf("Gerando novo arquivo de dados para teste...\n");
    gerar_arquivo_de_dados("dados.bin");

    long long raiz = constroi_arvore("dados.bin", "ArqIdx.bin", "ArqDados.bin");
    if (raiz == -1) {
    ...
    while (1) {
        ...
        switch (opcao) {
            case 1: {
                ...
                raiz = TARVBP_insere(raiz, &novo_aluno, "ArqIdx.bin", "ArqDados.bin");
            ...
            case 2:
                ...
                scanf("%12s", cpf_buffer);
                limpar_buffer_teclado();
                raiz = TARVBP_remove(raiz, cpf_buffer, "ArqIdx.bin");
                break;
            case 3:
                printf("\n--- Buscar Aluno ---\n");
                ...
                scanf("%12s", cpf_buffer);
                limpar_buffer_teclado();
                buscar_e_imprimir_dados(raiz, cpf_buffer, "ArqIdx.bin", "ArqDados.bin");
                break;
            case 0:
